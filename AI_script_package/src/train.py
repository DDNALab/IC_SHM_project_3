...
from paths import CHECKPOINTS_DIR, CONFIG_DIR, PROJECT_ROOT, TABLES_DIR, ensure_output_dirs
from utils import load_yaml, save_json, set_seed


def main() -> None:
    parser = argparse.ArgumentParser(description="QLoRA fine-tuning for Qwen3.5-4B on Project 3.")
    ...
    train_csv = Path(data_cfg.get("train_csv", TABLES_DIR / "train.csv"))
    val_csv = Path(data_cfg.get("val_csv", TABLES_DIR / "val.csv"))
    run_dir = Path(train_cfg.get("output_dir", CHECKPOINTS_DIR / run_name))
    train_csv = train_csv if train_csv.is_absolute() else PROJECT_ROOT / train_csv
    val_csv = val_csv if val_csv.is_absolute() else PROJECT_ROOT / val_csv
    ...
    parameter_summary["lora_target_module_count"] = len(lora_targets)
    parameter_summary["lora_target_modules"] = lora_targets
    save_json(parameter_summary, TABLES_DIR / f"{run_name}_model_parameters.json")
    model.print_trainable_parameters()

    train_dataset = DamageDataset(train_csv)
    eval_dataset = DamageDataset(val_csv)
    ...
    precision = model_cfg.get("precision", "bf16").lower()
    ...
    training_args = TrainingArguments(
        ...
        bf16=use_bf16,
        fp16=use_fp16,
        tf32=bool(train_cfg.get("tf32", True)) and torch.cuda.is_available(),
        gradient_checkpointing=bool(train_cfg.get("gradient_checkpointing", True)),
        optim=str(train_cfg.get("optim", "paged_adamw_8bit")),
        max_grad_norm=float(train_cfg.get("max_grad_norm", 1.0)),
        report_to=list(train_cfg.get("report_to", ["tensorboard"])),
    ...
    log_frame = pd.DataFrame(trainer.state.log_history)
    log_frame.to_csv(TABLES_DIR / f"{run_name}_training_log.csv", index=False)
    save_json(
        {
            "config": config,
        ...
        TABLES_DIR / f"{run_name}_training_summary.json",
    )
    print(f"Final adapter: {final_adapter_dir.resolve()}")
